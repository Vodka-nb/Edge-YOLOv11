...
from .utils.kalman_filter import KalmanFilterXYAH, KalmanFilterXYWH
...
class BYTETracker:
    ...
    def update(self, results, img=None):
        ...
        self.frame_id += 1
        activated_stracks = []   #保存当前帧匹配到持续追踪的轨迹
        refind_stracks = []     #保存当前帧匹配到之前目标丢失的轨迹
        lost_stracks = []      #保存当前帧没有匹配到目标的轨迹
        removed_stracks = []    #保存当前帧被移除的轨迹
        ...
        inds_second = inds_low & inds_high     #  筛选分数处于0.1<分数<阈值的
        ...
        # 分数比较低的目标框没有匹配到轨迹就会直接被扔掉，不会创建新的轨迹
        ...
        # 用match_thresh = 0.8过滤较小的iou，利用匈牙利算法进行匹配，得到matches, u_track, u_detectio

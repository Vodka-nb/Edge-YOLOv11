...
# 用于跟踪IOU计算的函数，直接修改默认参数可修改IOU计算方式

def batch_bbox_iou(boxes1: np.ndarray, boxes2: np.ndarray,
                   GIoU=False, DIoU=False, CIoU=True, EIoU=False,
                   SIoU=False, ShapeIoU=False, PIoU=False, PIoU2=False) -> np.ndarray:
    """
    计算两组边界框的批量 IoU，支持不同 IoU 计算方式

    Args:
        boxes1 (np.ndarray): (N, 4) 或 (N, 5) 的边界框
        boxes2 (np.ndarray): (M, 4) 或 (M, 5) 的边界框
        其他参数: 选择不同 IoU 计算方法

    Returns:
        np.ndarray: 形状为 (N, M) 的 IoU 结果矩阵
    """
    if len(boxes1) == 0 or len(boxes2) == 0:
        return np.zeros((len(boxes1), len(boxes2)), dtype=np.float32)

    # 转换 numpy -> torch.Tensor
    boxes1 = torch.tensor(boxes1, dtype=torch.float32) if isinstance(boxes1, np.ndarray) else boxes1
    boxes2 = torch.tensor(boxes2, dtype=torch.float32) if isinstance(boxes2, np.ndarray) else boxes2

    # 确保 boxes1 和 boxes2 的最后一个维度是 4 或 5（防止 shape 错误）
    assert boxes1.shape[-1] in [4, 5], f"boxes1 维度错误: {boxes1.shape}"
    assert boxes2.shape[-1] in [4, 5], f"boxes2 维度错误: {boxes2.shape}"

    # **关键修改：确保广播计算**
    boxes1 = boxes1[:, None, :]  # (N, 4) -> (N, 1, 4)
    boxes2 = boxes2[None, :, :]  # (M, 4) -> (1, M, 4)

    # 计算 IoU
    ious = bbox_iou(boxes1, boxes2, xywh=True,
                    GIoU=GIoU, DIoU=DIoU, CIoU=CIoU,
                    EIoU=EIoU, SIoU=SIoU, ShapeIoU=ShapeIoU,
                    PIoU=PIoU, PIoU2=PIoU2)

    # 移除最后一个维度（如果存在）并转换为 numpy
    return ious.squeeze(-1).numpy()  # 确保输出形状为 (N, M
